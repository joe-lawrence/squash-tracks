...
2. **`data/ghosting-model.json`** — machine source of truth; in particular:
   ...
   - **`rules.targetNotation.ballSideContinuity`** — consecutive beats: user `heading` = ghost landing; ghost striker = prior user landing; width flips only on **`Cross…`** and **`Boast`**.
   - **`shotConstraints`** — legal shots and good landing corners.
   - **`transitions`** — player reply weights (prior user shot name must match a matrix row key).
...
- **Width:** Only **`Cross…`** and **`Boast`** switch ball side **`L↔R`**; all other shots keep side.
